...
class CNN_Text(nn.Module):
    ...
    def __init__(self, args):
        ...
        print('alpha low boundary is,', self.args.lowb)
        ...
        if self.args.lstm:
            ...
            # self.rnn = nn.LSTM(
            self.rnn = nn.GRU(
                input_size=self.embed_dim, hidden_size=self.hidden_size, num_layers=2, batch_first=True,
                bidirectional=True)
        ...
        if self.args.selfensemble:
            ...
            if self.args.lstm:
                ...
                self.rnn_2 = nn.GRU(
                    input_size=self.embed_dim, hidden_size=self.hidden_size, num_layers=2, batch_first=True,
                    bidirectional=True)
                self.fc1_2 = nn.Linear(self.hidden_size * 2, self.class_num)
            else:
    ...
    def forward(self, x0, y0 = None, bacthSize = None):
        ...
        if self.args.lstm:
            ...
            # x = [self.dropout(F.relu(conv(x)).squeeze(3)) for conv in self.convs1]  # [(N, Co, W), ...]*len(Ks)  # add by Jianfeng with mutli layer dropout
        ...
        x = self.dropout(x)  # (N, len(Ks)*Co)     #exisiting in original Xuchao
        ...
        if self.args.selfensemble:
            ...
            x_2 = self.dropout_2(x_2)  # (N, len(Ks)*Co)     #exisiting in original Xuchao
