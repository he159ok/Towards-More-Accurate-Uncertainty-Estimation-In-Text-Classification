...
class Amazon_DataSet_XLnet:
    ...
    def generate_data(self, val_ratio=.1, shuffle=True):
        ...
        if os.path.exists(self.data_dir+'AmazonXLnetTrFea2.npy') and os.path.exists(self.data_dir+'AmazonXLnetTeFea2.npy')\
                and os.path.exists(self.data_dir+'AmazonXLnetVaFea2.npy'):
            print('we can load XLnetFea directly')
            x_train = np.load(self.data_dir+'AmazonXLnetTrFea2.npy')
            x_test = np.load(self.data_dir+'AmazonXLnetTeFea2.npy')
            x_val = np.load(self.data_dir+'AmazonXLnetVaFea2.npy')
            y_train = np.load(self.data_dir+'AmazonXLnetTrLab2.npy')
            y_test = np.load(self.data_dir + 'AmazonXLnetTeLab2.npy')
            y_val = np.load(self.data_dir + 'AmazonXLnetVaLab2.npy')
            ...
            np.save(self.data_dir + 'AmazonXLnetTrFea2.npy', x_train)
            np.save(self.data_dir + 'AmazonXLnetTeFea2.npy', x_test)
            np.save(self.data_dir + 'AmazonXLnetVaFea2.npy', x_val)
            np.save(self.data_dir + 'AmazonXLnetTrLab2.npy', y_train)
            np.save(self.data_dir + 'AmazonXLnetTeLab2.npy', y_test)
            np.save(self.data_dir + 'AmazonXLnetVaLab2.npy', y_val)
            print('save 6 files for Amazon successully!')
            return (x_train, y_train), (x_val, y_val), (x_test, y_test)
    ...
    def format_data(self, data_frame, vocab_idx):
        ...
        tokenizer = XLNetTokenizer.from_pretrained(self.pretrained_weights)
        model_class = XLNetModel
        XLnet = model_class.from_pretrained(pretrained_weights)
        jsq = 0
        for index in data_frame.index:
            if jsq % 1000 == 0:
                print(jsq)
            jsq += 1
            sample = data_frame.Text[index]
            # text_word_list = [self.convertSent2WordIds(sentence, vocab_idx) for sentence in
            #                   document]
            # sample = train_df.Text[0]
            res = ""
            for subsen in sample:
                mid = " "
                res += mid.join(subsen)
            ...
            # if len(res) > 512:
            #     res = res[:512]
            input_ids1 = torch.tensor([tokenizer.encode(res, add_special_tokens=True)])
            # input_ids1 = torch.tensor([tokenizer.encode(res, add_special_tokens=True)]).float()

            # 此为存hidden feature from Bert的代码
            with torch.no_grad():
                last_hidden_states = XLnet(input_ids1)[0]
                last_hidden_states = last_hidden_states[:, -1, :]
            input_ids = np.array(last_hidden_states)

            # 此为存tokenization ID 的代码
            # if len(input_ids[0]) < 512:
            #     miss = torch.tensor([0] * (512-len(input_ids[0]))).float().unsqueeze(0)
            #     input_ids = torch.cat((input_ids, miss), 1)

            if len(input_ids) != 1:
                raise ValueError
            ids_document.append(input_ids)
            # labels
            ids_labels.append(data_frame.Class[index])
            # if jsq >= 200:
        # final_res = torch.cat(ids_document, dim = 0).int()
        # return final_res, np.array(ids_labels)
        # return ids_document, np.array(ids_labels)
    ...
    def gen_minibatch(self, tokens, labels, mini_batch_size, args, shuffle=True):
        ...
        for token, label, excerpt in self.iterate_minibatches(tokens, labels, mini_batch_size, shuffle=shuffle):
            ...
            token = Variable(torch.from_numpy(token), requires_grad=True)
            if args.cuda == True:
